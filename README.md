# SaaS-Data-Analytics
saas-analytics/
│
├── README.md                  # [POST] Your project homepage & instructions
├── requirements.txt           # [POST] Python packages needed
│
├── sql/                       # [POST] SQL scripts
│   ├── schema.sql             # DDL to create tables
│   ├── sample_data.sql        # INSERT statements for demo SaaS data
│   └── analysis_queries.sql   # Example queries (MRR, churn, revenue)
│
├── python/                     # [POST] Python scripts
│   ├── load_data.py           # Loads CSVs into SQLite database
│   └── analysis.ipynb         # Jupyter notebook for Pandas analysis & charts
│
├── data/                       # [POST] Optional small CSV files for demo
│   ├── customers.csv
│   ├── subscriptions.csv
│   └── invoices.csv
│
└── docs/                       # [OPTIONAL] Screenshots, diagrams, extra documentation
